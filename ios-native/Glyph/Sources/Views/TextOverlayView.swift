...
struct TextOverlayView: View {
    ...
    var body: some View {
        Text(layer.text.isEmpty ? " " : layer.text)
            ...
            .onTapGesture { if isSelected { onEdit() } else { onSelect() } }
            .accessibilityElement(children: .ignore)
            .accessibilityLabel("Text layer: \(layer.text.isEmpty ? "empty" : layer.text)")
            .accessibilityHint(isSelected ? "Double-tap to edit. Use actions for more options." : "Double-tap to select.")
    ...
    private func handleDoubleTap() {
        ...
        // If already at a cardinal, reset to 0°
            ...
            SimultaneousGesture(
                ...
                RotationGesture()
                    .updating($gestureRotation) { v, s, _ in
                        s = v
                        // Haptic tick at cardinal angles during live rotation
                        let total = layer.rotation + v
                        if RotationSnapEngine.snap(total) != nil {
                            haptics.rotationSnap()
                        }
                    }
                    .onEnded { value in
